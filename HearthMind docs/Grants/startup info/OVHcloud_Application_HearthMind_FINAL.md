...
United States, Wyoming  Sheridan (registered) — operating from Spokane, Washington

**6. What is your company's physical address?**
405 S. Maple St # 17, Spokane WA 99201
...
HearthMind builds privacy-first, emotionally intelligent AI systems designed to help individuals navigate complex life transitions involving benefits, disability, housing, and care systems.

Our first product, Navigator, is an AI-guided discovery and intake platform that helps users understand eligibility rules, required documentation, and next steps without cognitive overload.

Under the hood, HearthMind focuses on local-first and hybrid AI architectures, multi-agent systems, and continuity-aware models designed to operate responsibly in sensitive, high-trust domains.
...
Yes. Navigator has a working prototype with live AI interaction, real-world test scenarios, and active demonstrations.
...
$0 MRR currently. Forecast focuses on pilot programs, institutional partnerships, and grant-backed deployments rather than early consumer revenue, aligning with a sustainable, compliance-aware rollout.
...
This architecture is designed to transition cleanly from local development to dedicated bare-metal cloud infrastructure without re-platforming.

**27. Why are you considering migrating your infrastructure to OVHcloud?**
Three primary reasons:

1. **Data sovereignty and trust**: HearthMind handles sensitive contextual data related to disability, mental health, and life instability. OVHcloud's approach to data sovereignty, transparent infrastructure, and bare-metal isolation aligns with our ethical and technical requirements in ways hyperscalers do not.

2. **GPU access for training**: LoRA fine-tuning for personalized AI models requires dedicated GPU compute beyond our local capacity. Bare-metal GPU servers give us the control and performance needed.
...
- **High-memory compute instances** — For running inference on larger models (14B-70B parameters), requiring high-memory GPU configurations suitable for large-model inference and fine-tuning
...
Usage is expected to scale in step with pilot deployments rather than burst unpredictably.
...
We are specifically interested in OVHcloud as a long-term infrastructure partner rather than a short-term credit provider, particularly for GPU bare-metal workloads and sovereignty-sensitive deployments.
